...
const KICK_SNIPPET = preload("uid://dcrnkxooyq5i4")
const M_1_SNIPPET = preload("uid://b3s61uvjw0npg")
...
var current_place_soundsnippet: SoundSnippet = null
...
func set_snippet_type(sound_type: SoundSnippet.SoundType) -> void:
	if is_instance_valid(current_place_soundsnippet):
		current_place_soundsnippet.queue_free()

	match sound_type:
		SoundSnippet.SoundType.KICK:
			current_place_soundsnippet = KICK_SNIPPET.instantiate()
			
		SoundSnippet.SoundType.M1:
			current_place_soundsnippet = KICK_SNIPPET.instantiate()
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		if iemb.is_pressed() && iemb.button_index == MOUSE_BUTTON_RIGHT:
			for os: SoundSnippet in current_place_soundsnippet.obstructing_snippets:
				os.queue_free()
